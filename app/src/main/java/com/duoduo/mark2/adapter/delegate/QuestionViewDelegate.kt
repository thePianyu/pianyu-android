 ...
 */
class QuestionViewDelegate(val listener: OnTopicClickListener, val imageClickListener: MarkdownUtils.ImageClickListener) :
    ItemViewDelegate<Question, QuestionViewDelegate.ViewHolder>() {
    ...
    override fun onCreateViewHolder(context: Context, parent: ViewGroup): ViewHolder {
        ...
        return ViewHolder(LayoutInflater.from(context).inflate(R.layout.item_question_content, parent, false), markdown)
    }

    override fun onBindViewHolder(holder: ViewHolder, item: Question) {
        //holder.markwon.setMarkdown(holder.textView, item.content_markdown ?: "null") // 正文
        val markdown = holder.markwon.toMarkdown(item.content_markdown)
        holder.markwon.setParsedMarkdown(holder.textView, markdown)
